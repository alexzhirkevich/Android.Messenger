...
open class AuthProviderImp @Inject constructor(
    ...
    override fun setNotificationToken(token: String): Completable =
        FirebaseDatabase.getInstance().reference.child(FirebaseProvider.COLLECTION_USERS).child(usersProvider.currentUserId)
            .child(FirebaseProvider.FIELD_NOTIFY_TOKEN).setValue(token).toCompletable()

    private val userOnlineReference : DatabaseReference by lazy {
        FirebaseDatabase.getInstance().reference
            .child(FirebaseProvider.COLLECTION_USERS).child(FirebaseAuth.getInstance().currentUser!!.uid).child(
                FirebaseProvider.FIELD_ONLINE
            )
    }
}