...
class TestChannelsProvider : ChannelsProvider {
    ...
    override fun get(id: String): Observable<IChannel> =
            Observable.just(com.community.messenger.common.entities.imp.Channel(id = id))


    override fun getAll(collection: IUser, limit: Int): Observable<Collection<IChannel>> {
        val list = mutableListOf<com.community.messenger.common.entities.imp.Channel>()
        for (i in 1..limit){
            list.add(
                com.community.messenger.common.entities.imp.Channel(
                    id = "test$i",
                    name = "Channel Name",
                    lastPostId = "test",
